...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		triangle(45, 265, 50, 255, 55, 265);
		
		fill(0, 0, 0);
		text("5.0+ Magnitude", 75, 110);
		text("4.0+ Magnitude", 75, 140);
		text("Below 4.0", 75, 170);
		text("Land Quake", 75, 200);
		text("Ocean Quake", 75, 230);
		text("Cities", 75, 260);
	}
	...
	private void printQuakes() 
	{
		...
		for (Marker m : quakeMarkers) { 
			...
			if( (boolean)em.isOnLand()) {
				String markerCountry = (String)em.getProperty("country");
				for (Marker cm : countryMarkers) { 
					String name = (String)cm.getProperty("name");
					if(markerCountry == name) {
						counter.put(name, counter.containsKey(name) ? counter.get(name) + 1 : 1);
					}
				}
			}
			else
		...
		System.out.println(" Countries with earthquakes:");
	    System.out.print("\t" + counter);
	    System.out.println(" Earthquakes in ocean");
	    System.out.print("\t" + oceanQuakeCounter);
		//        
